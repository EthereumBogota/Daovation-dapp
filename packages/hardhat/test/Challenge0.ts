...
describe("🚩 Challenge 0: 🎟 Simple NFT Example 🤓", function () {
  ...
  describe("DaovationNFT", function () {
    const contractAddress = process.env.CONTRACT_ADDRESS;
    if (contractAddress) {
      it("Should connect to external contract", async function () {
        myContract = await ethers.getContractAt("DaovationNFT", contractAddress);
        console.log("     🛰 Connected to external contract", myContract.address);
      });
    } else {
      it("Should deploy DaovationNFT", async function () {
        const DaovationNFT = await ethers.getContractFactory("DaovationNFT");
        myContract = await DaovationNFT.deploy();
      });
    }
