...
export const MyHoldings = () => {
  ...
  useEffect(() => {
    const updateMyCollectibles = async (): Promise<void> => {
      if (myTotalBalance === undefined || DaovationNFTContract === undefined || connectedAddress === undefined) return;
      ...
      for (let tokenIndex = 0; tokenIndex < totalBalance; tokenIndex++) {
        try {
          const tokenId = await DaovationNFTContract.read.tokenOfOwnerByIndex([
            connectedAddress,
            BigInt(tokenIndex.toString()),
          ]);

          const tokenURI = await DaovationNFTContract.read.tokenURI([tokenId]);
